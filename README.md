...
| **Architecture Decision Records (ADRs)** | Formal ADRs (`.md` / `.docx` / `.pdf`) capturing the load-bearing architecture decisions with Context / Decision / Consequences / Alternatives-considered, each traced to the Decision Log (D1–D11) and hard constraints. Available both as a **consolidated document** (`docs/acrme_architecture_decision_records.*`) and as **four individual standalone ADRs** (`docs/adr/`): ADR-001 Region Selection (sequential Prod→NonProd→DR placement, region classification, Scenario 1/2 input modes, EC-1..EC-4 exceptions, VR-1..VR-11, corrected scoring), ADR-002 Quota & Capacity Management (two quota groups, DR-floor accounting formulas, `groupType` FC-11 preview status), ADR-003 Capacity Management during DR (NonProd/DR co-location, full five-state engine machine, three-tier emergency transfer), and ADR-004 Forecast & Increase of Capacity and Quota (approval-gated growth, 10-step steady-state lifecycle, auto-decrease exclusion). |
