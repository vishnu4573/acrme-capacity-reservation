...
| **Architecture Decision Records (ADRs)** | Formal, **self-contained** ADRs (`.md` / `.docx` / `.pdf`) capturing the load-bearing architecture decisions with Context / Decision / Consequences / Alternatives-considered. Each record is reference-free — all normative detail is inlined so no companion document is required to read it — and includes **rendered architecture diagrams**. Available both as a **consolidated document** (`docs/acrme_architecture_decision_records.*`) and as **four individual standalone ADRs** (`docs/adr/`): ADR-001 Region Selection (sequential Prod→NonProd→DR placement, region classification, geography vs. specific-region input modes, restricted-region exception workflow, corrected scoring), ADR-002 Quota & Capacity Management (two quota groups, DR-floor accounting formulas, preview `groupType` model), ADR-003 Capacity Management during DR (NonProd/DR co-location, full five-state engine machine, three-tier emergency transfer), and ADR-004 Forecast & Increase of Capacity and Quota (approval-gated growth, 10-step steady-state lifecycle, auto-decrease exclusion). Diagram sources and PNGs live in `docs/adr/diagrams/`. |
