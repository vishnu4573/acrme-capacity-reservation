...
| **Architecture Decision Records (ADRs)** | Four formal ADRs (`.md` / `.docx` / `.pdf`) capturing the load-bearing architecture decisions with Context / Decision / Consequences / Alternatives-considered: ADR-001 Region Selection (sequential Prod→NonProd→DR placement, separation as a hard constraint), ADR-002 Quota & Capacity Management (two quota groups, env-type PS formulas), ADR-003 Capacity Management during DR (dr_ratio_max floor sizing, two operating modes, NonProd/DR co-location), and ADR-004 Forecast & Increase of Capacity and Quota (three-tier emergency transfer, auto-increase triggers) — each traced to the Decision Log (D1–D11) and hard constraints. |
