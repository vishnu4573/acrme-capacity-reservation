...
| **CAP-023** (extends CAP-011) | **One zonal CRG per environment** (zones fixed at creation; one reservation per VM size per zone). A no-zone group is the pinned-zone exception, not a region-wide pool. **[v2.5]** | ✅ COVERED | Provisioning/topology; baseline CAP-023; naming per OPS-006. TDD §5.2 still describes the earlier regional + per-AZ layout and needs a follow-up edit |
