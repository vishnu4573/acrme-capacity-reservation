...
> ## ⚠️ SUPERSEDED (7 Sep 2026) — see Baseline v2.4
> This is the **v2.1** consolidated requirements baseline and is **superseded** by **`acrme_requirements_baseline_v2_4.md` (version v2.4)**, which is the **single source of truth**. In particular, the **region scope below is outdated**: v2.4 supports **five geographies** (US, EU, Australia, Asia Pacific, Middle East). References here to APAC/Australia/Japan being "descoped or on hold" and to a "North America and Europe focused" strategy are **no longer current** — Australia and Asia Pacific are **in-scope two-region geographies**, and the US is the only three-region geography. Two-region geographies co-locate CVAL and DR (PLC-010a); the Middle East is the sole geography with DR `DR_NOT_OFFERED` (DEC-001). Retained for historical reference only.
