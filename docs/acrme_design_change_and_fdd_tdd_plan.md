...
| **ADR-005 (new)** | Distributed DR Reference Model — Section 12A topology, `SourceDestinationDRIndex`, bidirectional mapping, max-not-sum boundary, single-failure assumption, observability. |
...
| F6 | Distributed DR Reference Model (Section 12A) | **Reuse** existing `acrme_three_region_capacity_model.png` |
...
8. Placement scoring & forecasting (Section 28) — PS_Prod / PS_NonProd / PS_DR, weights α=0.30/β=0.20/γ=0.25/δ=0.15/ε=0.10, normalization, determinism/replay.
...
| T5 | Data model / ERD incl. seed record + **SourceDestinationDRIndex** + CVAL earmark | ERD | Section 34, DAT-002 |
| T6 | Five-state engine machine | State | Section 29; reuse `adr003_state_machine` |
...
| T10 | Three-tier emergency transfer escalation | Flow | Section 32; reuse `adr003_transfer_tiers` |
...
| T13 | Distributed DR Reference Model (Section 12A) | **Reuse** `acrme_three_region_capacity_model.png` | Section 12A |
