...
| **ADR-005** | Distributed DR Reference Model | §12A topology, reciprocal many-to-many roles, `SourceDestinationDRIndex` (bidirectional), max-not-sum destination boundary + overcommit ratio/gap, single-failure assumption, DR earmark in single pool, observability contract | `acrme_adr_005_distributed_dr_reference_model.{md,pdf,docx}` |
