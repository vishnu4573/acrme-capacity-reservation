#!/usr/bin/env python3
"""ACRME Region-Selection WHAT-IF — 4-REGION DR MODEL SCENARIO.

This is a WHAT-IF variant exploring the impact of upgrading US and EU from their
baseline v2.4 models (US 3-region, EU 2-region) to a 4-REGION distribution model
for both geographies.

SCENARIO CHANGES:
- US: 3-region → 4-region (all 4 US Standard regions now candidates)
- EU: 2-region → 4-region (all 4 EU Standard regions now candidates)
- Australia, Asia Pacific: remain 2-region (unchanged)
- Middle East: remains cross-geo DR (unchanged)

IMPACT:
- PLC-010a (CVAL/DR co-location) NO LONGER APPLIES to US or EU
- HC-6/HC-7 co-location checks DISABLED for 4-region geographies
- γ (distribution fairness) component normalizes over 4 regions instead of 2/3
- More region candidates = finer-grained placement scoring

This mockup does NOT modify the baseline requirements — it is a sandbox for
exploring the 4-region model impact before any formal REG-003 amendment.
"""
...
OUT  = "/home/ubuntu/acrme-capacity-reservation/Mockups/acrme_region_selection_whatif_4region.xlsx"
...
GEO_MODEL={'US':('4-region','US'),'EU':('4-region','EU'),'Australia':('2-region','Australia'),
...
rows=[
 ...
 ("",None),
 ("4-REGION MODEL SCENARIO — WHAT-IF EXPLORATION",H2,SUBFILL),
 ("• **US**: upgraded from 3-region (baseline) → **4-region model**. All 4 US Standard regions (West US 3, Central US, "
  "Canada Central, East US 2) now compete for placement. **PLC-010a co-location NO LONGER APPLIES** — Prod / CVAL / DR "
  "can each occupy separate regions.",None,PatternFill("solid",fgColor="E7E6E6")),
 ("• **EU**: upgraded from 2-region (baseline) → **4-region model**. All 4 EU Standard regions (Switzerland North, "
  "Sweden Central, North Europe, West Europe) now compete. **CVAL and DR no longer co-locate** — they occupy distinct "
  "regions, improving blast-radius isolation.",None,PatternFill("solid",fgColor="E7E6E6")),
 ("• **Australia, Asia Pacific**: remain 2-region (unchanged from baseline). CVAL/DR co-location still applies.",None),
 ("• **Middle East**: remains cross-geo DR (unchanged). Prod+CVAL local in Middle East; DR in Europe.",None),
 ("• **Impact on HC gates**: HC-6/HC-7 (CVAL/DR co-location checks) disabled for US and EU 4-region geographies. "
  "HC-3 (individual environment checks) still applies to all regions.",None,WARNFILL),
 ("• **Middle East** regions (Saudi Arabia East, UAE North) remain RESTRICTED — production-only; explicit Prod region "
  "required on Setup (cell B8). DR still auto-selects cross-geo in a Standard Europe region.",None,RESTFILL),
 ("",None),
 ...
 ("7. Scoring_Prod / 8. Scoring_CVAL / 9. Scoring_DR — PS component breakdown (α/β/γ/δ/ε raw + clamped), AutoSelect "
  "flag, Candidate (PS if eligible, −1 if not), and Rank (1 = best candidate, blank = not a candidate for that mode). "
  "Winner summary in cols T–U.",None),
 ("10. Result — final placement (Prod / CVAL / DR), readiness, DR sizing (max-not-sum), and a Ranked Candidate "
  "Regions table (cols H–P) showing all candidate regions ordered by PS for each mode.",None),
 ("",None),
 ...
 ("Rank formula (Scoring sheets, col S): Rank=COUNTIF(Candidate_range,\">\"&this_candidate)+1. Non-candidates "
  "(Candidate=-1) are blank. Rank 1 = region with the highest PS score among eligible candidates. Because PS is a "
  "continuous float, ties are effectively impossible.",None),
...
geos=[("US","4-region","US"),("EU","4-region","EU"),("Australia","2-region","Australia"),
      ("Asia Pacific","2-region","Asia Pacific"),("Middle East","cross-geo","EU")]
...
for i in range(NR):
    ...
    # HC-6/HC-7: DISABLED for 4-region geographies (US, EU) — CVAL and DR no longer co-locate
    hc.cell(row=xr,column=8,value=f'=IF(OR(B{xr}="US",B{xr}="EU"),"N/A (4-region)",IF(Capacity_Usage!S{cur}+Capacity_Usage!N{cur}>=Policy!$B$13,"PASS","FAIL"))')
    hc.cell(row=xr,column=9,value=f'=IF(OR(B{xr}="US",B{xr}="EU"),"N/A (4-region)",IF(Capacity_Usage!P{cur}+Setup!$B$7<=Capacity_Usage!O{cur}-Policy!$B$13,"PASS","FAIL"))')
    hc.cell(row=xr,column=10,value=f'=IF(E{xr}="PASS","YES","NO")')
    # Eligible NonProd/DR: HC-6/HC-7 automatically satisfied for 4-region (treat N/A as PASS)
    hc.cell(row=xr,column=11,value=f'=IF(AND(F{xr}="PASS",OR(I{xr}="PASS",I{xr}="N/A (4-region)")),"YES","NO")')
    hc.cell(row=xr,column=12,value=f'=IF(AND(G{xr}="PASS",OR(H{xr}="PASS",H{xr}="N/A (4-region)")),"YES","NO")')
...
def build_scoring(name,alpha_num,alpha_den,beta_num,beta_den,delta_expr,elig_col,scope_ref,exclude_prod,title):
    ...
    # Col layout: A=Region B=Geo C=InScope D=Eligible E=AutoSel F=total_cust
    #             G-P = α/β/γ/δ/ε raw+clamped pairs   Q=PS   R=Candidate   S=Rank
    ...
    HRr=3; rank_top=HRr+1; rank_bot=HRr+NR  # 4 and 3+NR
    ...
    for i in range(NR):
        ...
        # Rank among candidates: COUNTIF counts candidates with a HIGHER score; non-candidates (R=-1)
        # cannot beat any positive score so they are never counted. Result: Rank 1 = best candidate.
        s.cell(row=xr,column=19,value=f'=IF(R{xr}<0,"",COUNTIF($R${rank_top}:$R${rank_bot},">"&R{xr})+1)')
    ...
    s.column_dimensions["A"].width=20; s.column_dimensions["B"].width=13; s.column_dimensions["S"].width=6
    return s,HRr
...
# ---- Ranked Candidates table (H3:P3+NR+2) ------------------------------------------
# Three side-by-side ranked lists (Prod / CVAL / DR) using the Rank column (S) on each
# scoring sheet. MATCH(k, Scoring!$S$4:$S$17, 0) finds the row ranked k-th; IFERROR
# returns "" when rank k doesn't exist (fewer candidates than NR). No array formulas.
rank_row_start = 3
rk_scoring_refs = [
    ("Prod",  "Scoring_Prod",  8),   # start col H
    ("CVAL",  "Scoring_CVAL", 11),   # start col K
    ("DR",    "Scoring_DR",   14),   # start col N
]
rs.cell(row=rank_row_start, column=8, value="RANKED CANDIDATE REGIONS  (Rank 1 = highest PS; blank = not a candidate for this mode)")
rs.merge_cells(f"H{rank_row_start}:P{rank_row_start}")
rs["H3"].font=H2; rs["H3"].fill=SUBFILL; rs["H3"].alignment=Alignment(horizontal="left",vertical="center")
# Mode sub-headers
for mode, _, sc in rk_scoring_refs:
    c=rs.cell(row=rank_row_start+1,column=sc,value=mode)
    rs.merge_cells(f"{get_column_letter(sc)}{rank_row_start+1}:{get_column_letter(sc+2)}{rank_row_start+1}")
    c.font=BOLD; c.alignment=CTR; c.fill=HEADFILL; c.font=WHITEB
# Column headers
col_hdrs=["Rank","Region","PS"]
for _,_,sc in rk_scoring_refs:
    for j,h in enumerate(col_hdrs):
        c=rs.cell(row=rank_row_start+2,column=sc+j,value=h)
        c.font=WHITEB; c.fill=HEADFILL; c.alignment=CTR; c.border=BORDER
# Data rows — one per rank position (1..NR); IFERROR blanks out ranks with no candidate
for k in range(1, NR+1):
    dr=rank_row_start+2+k
    for mode, sheet, sc in rk_scoring_refs:
        # Rank number (show k only if a region exists at that rank)
        rs.cell(row=dr,column=sc,
            value=f'=IFERROR(IF(INDEX({sheet}!$S${top}:$S${bot},MATCH({k},{sheet}!$S${top}:$S${bot},0))="","",{k}),"")')
        # Region name
        rs.cell(row=dr,column=sc+1,
            value=f'=IFERROR(INDEX({sheet}!$A${top}:$A${bot},MATCH({k},{sheet}!$S${top}:$S${bot},0)),"")')
        # PS score
        rs.cell(row=dr,column=sc+2,
            value=f'=IFERROR(INDEX({sheet}!$Q${top}:$Q${bot},MATCH({k},{sheet}!$S${top}:$S${bot},0)),"")')
        for off in range(3):
            cc=rs.cell(row=dr,column=sc+off); cc.border=BORDER; cc.alignment=CTR
        rs.cell(row=dr,column=sc+2).number_format="0.000"
# Column widths for ranked table columns H-P
for col,w in [(8,6),(9,20),(10,8),(11,6),(12,20),(13,8),(14,6),(15,20),(16,8)]:
